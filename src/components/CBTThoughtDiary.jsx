...
export default function CBTThoughtDiary() {
  ...
  const handleReframe = async () => {
    ...
    try {
      ...
      const text = await callClaude(`Someone wrote this thought in their CBT diary: "${thought}". ${factContext}\n\nUsing warm, gentle CBT techniques, please:\n1. Gently validate their experience first\n2. Softly explore whether the thought might be a cognitive distortion — name it kindly\n3. Offer a balanced, compassionate reframe in 2-3 sentences\n\nKeep it warm and non-clinical.`, null, 500)
      setReframe(text)
      setView('result')
    } catch {
  ...
  const saveEntry = () => {
    const entry = { id: Date.now(), thought, reframe, isFact, date: new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) }
    const updated = [entry, ...entries]
    setEntries(updated)
    localStorage.setItem('cbt_entries', JSON.stringify(updated))
    setThought(''); setReframe(''); setIsFact(null); setView('write')
  }

  const factOpts = [
    { val: true, label: '📌 Feels like a fact' },
    { val: false, label: '💭 Feels like a feeling' },
        ...
        <AnimatePresence mode="wait">
          {view === 'write' && (
            <motion.div key="write" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              ...
              <div className="card" style={{ marginBottom: 16 }}>
                ...
                <button onClick={handleReframe} disabled={!thought.trim() || loading} className="btn btn-primary" style={{ width: '100%', fontSize: '1rem', padding: '14px' }}>
          ...
          {view === 'past' && (
            <motion.div key="past" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              <div className="page-header"><h1>📋 Past Entries</h1><p>Your journey of reframing</p></div>
              <button onClick={() => setView('write')} className="btn btn-primary" style={{ width: '100%', marginBottom: 20 }}>+ New Entry</button>
              {entries.length === 0 ? (
                <div className="card" style={{ textAlign: 'center', padding: 40 }}><p>No entries yet. Start reframing today.</p></div>
              ) : (
